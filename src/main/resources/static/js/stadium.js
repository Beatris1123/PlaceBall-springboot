/**
 * stadium.js - 야구장 다이아몬드 Canvas 렌더러
 *
 * 원본: client/src/components/StadiumDiamondMap.tsx
 * React useEffect + Canvas API → 순수 JavaScript 변환
 */

/**
 * 야구장 다이아몬드 맵 그리기
 * 원본: export default function StadiumDiamondMap({ zones })
 *
...
function drawStadiumDiamondMap(zones) {
  ...
  // ── 색상 헬퍼 ──
  // 원본: const getGlowColor = (zone: Zone | undefined) => { ... }
...
/**
 * 구역 게이지 업데이트
 * 원본: zones.map(zone => <div key={zone.id}>...<div style={{width: `${zone.occupationRate}%`}}/></div>)
 *
 * @param {Array} zones - Zone[] 배열
 */
function updateZoneGauges(zones) {
  const container = document.getElementById('zone-gauges');
  if (!container) return;

  // 기존 게이지가 있으면 업데이트, 없으면 새로 생성
  if (container.children.length === 0) {
    container.innerHTML = zones.map((zone) => `
      <div class="zone-gauge-row" id="gauge-row-${zone.id}">
        <div class="zone-label">
          ${zone.owner === 'kia' ? '🔴' : '🔵'} 구역${zone.id + 1}
        </div>
        <div class="gauge-track">
          <div class="gauge-fill ${zone.owner}" id="gauge-fill-${zone.id}"
               style="width:${zone.occupationRate}%"></div>
        </div>
        <div class="gauge-pct" id="gauge-pct-${zone.id}">${zone.occupationRate}%</div>
      </div>
    `).join('');
  } else {
    // 점령률만 업데이트 (DOM 재생성 없이)
    zones.forEach((zone) => {
      const fill = document.getElementById(`gauge-fill-${zone.id}`);
      const pct  = document.getElementById(`gauge-pct-${zone.id}`);
      if (fill) fill.style.width = `${zone.occupationRate}%`;
